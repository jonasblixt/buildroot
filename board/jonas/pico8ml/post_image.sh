...
gen_boot_bpak()
{
    ...
    $BPAK add $IMG --meta pb-load-addr --from-string 0x43000000 --part-ref kernel \
                          --encoder integer $V

    $BPAK add $IMG --meta pb-load-addr --from-string 0x42f00000 --part-ref dt \
                        --encoder integer $V

    $BPAK add $IMG --meta pb-load-addr --from-string 0x44000000 --part-ref ramdisk \
                        --encoder integer $V

    $BPAK add $IMG --meta pb-load-addr --from-string 0x910000 --part-ref atf \
                        --encoder integer $V
    ...
    $BPAK add $IMG --part dt \
                   --from-file $BINARIES_DIR/imx8mq-pico-pi-pb.dtb $V
